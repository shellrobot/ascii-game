...
class gameRule:
    ...
    def set(var,value,ext1="",ext2=""):
        """Changes variables in the game easily without editing code"""

        if var == "speed": # Sets the speed of the game
            gameRule.speed = value
        elif var == "gameRule.playerChar":
            gameRule.playerChar = value
        elif var == "gameRule.borderChar":
            gameRule.borderChar = value
        elif var == "wallChar":
            wall.setWall(ext1,ext2)
        elif var == "blockChar":
            gameRule.blockChar = value
...
def createMap(x,y):
    ...
    for i in range(y):
        ...
        for o in range(x):
            if o == 0: # Left side border
                area[i].append(gameRule.borderChar+gameRule.borderChar)
    ...
    for i in range(map.x+3): 
        area[0].append(gameRule.borderChar) 
        area[len(area)-1].append(gameRule.borderChar) 
...
def printMap():
    ...
    for yPos, y in enumerate(area):
        for xPos, x in enumerate(area[yPos]):
            ...
            try: 
                if area[yPos][xPos] == gameRule.borderChar and area[yPos][xPos+1] == gameRule.borderChar:
                    areaString = areaString + str(gameRule.borderChar) # Prints double border char
...
def spawnPlayer(x,y):
    ...
    area[y][x] = gameRule.playerChar # Spawns player
...
def spawnObj(x="x",y="y"):
    ...
    block.x = blockx
    block.y = blocky
    area[blocky][blockx] = gameRule.blockChar
    return 1
...
def move(direction="Direction"):
    ...
    if direction.upper() == 'W': # Move Up (-Y)
        if player.y <= 1:
            ...
            player.y-=1
            area[player.y][player.x] = gameRule.playerChar # Set players new position
        ...
        if player.y >= map.y:
            ...
            player.y+=1
            area[player.y][player.x] = gameRule.playerChar
